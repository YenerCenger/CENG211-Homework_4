...
public class BoxPuzzle {
    ...
    private class Menu {
        ...
        private Direction[] getValidDirections(int row, int col) {
            ...
            if (top && left) return new Direction[]{Direction.RIGHT, Direction.DOWN};
            if (top && right) return new Direction[]{Direction.LEFT, Direction.DOWN};
            if (bottom && left) return new Direction[]{Direction.RIGHT, Direction.UP};
            if (bottom && right) return new Direction[]{Direction.LEFT, Direction.UP};
            
            // Non-corner edges have one option
            if (top) return new Direction[]{Direction.DOWN};
            if (bottom) return new Direction[]{Direction.UP};
            if (left) return new Direction[]{Direction.RIGHT};
            if (right) return new Direction[]{Direction.LEFT};
        ...
        private boolean hasValidEdgeBox() {
            ...
            for (int c = 0; c < BoxGrid.SIZE; c++) {
                if (!(grid.getBoxAt(0, c) instanceof FixedBox)) return true;
                if (!(grid.getBoxAt(BoxGrid.SIZE - 1, c) instanceof FixedBox)) return true;
            }
            // Check left and right columns (excluding corners already checked)
            for (int r = 1; r < BoxGrid.SIZE - 1; r++) {
                if (!(grid.getBoxAt(r, 0) instanceof FixedBox)) return true;
                if (!(grid.getBoxAt(r, BoxGrid.SIZE - 1) instanceof FixedBox)) return true;
            }
            return false;
        }
        ...
        private void displayBoxCube(int row, int col) {
            ...
            System.out.println("-------------");
            System.out.println("| " + layout[1][0] + " | " + layout[1][1] + " | " + layout[1][2] + " |"); // Left, Top, Right
            System.out.println("-------------");
        ...
        private void printRollMessage(Direction dir, boolean hitFixedBox) {
            ...
            if (hitFixedBox) {
                System.out.println("The chosen box and any box on its path have been rolled " + dirWord + " until a FixedBox has been reached. The new state of the box grid:\n");
            } else {
                System.out.println("The chosen box and any box on its path have been rolled " + dirWord + ". The new state of the box grid:\n");
            }
        }
        ...
        private Direction selectRollDirection(int row, int col) {
            ...
            while (true) {
                String choice = getInput("The chosen box can be rolled to either [1] " + dir1Word + " or [2] " + dir2Word + ": ");
        ...
        private boolean handleStage2Open() {
            ...
            while (true) {
                ...
                if (box.isOpen()) {
                    System.out.println("INCORRECT INPUT: This box has already been opened. Please reenter the location: ");
                    continue;
                }
                ...
                try {
                    ...
                    System.out.println("\nThe box on location R" + (coord.row + 1) + "-C" + (coord.col + 1) + " is opened. It contains a SpecialTool --> " + toolName);
                    
                    // Use the tool
                    handleToolUsage(tool, coord.row, coord.col);
                    return true;
         ...
         * @param tool The tool to use
         * @param openedRow The row where the box was opened (unused, kept for potential future use)
         * @param openedCol The column where the box was opened (unused, kept for potential future use)
         */
            ...
            if (tool instanceof tools.MassRowStamp) {
                ...
                while (true) {
                    ...
                    if (coord != null && isValidCoordinate(coord.row, coord.col)) {
                        ...
                        System.out.println("\nTop sides of the chosen box (R" + (coord.row + 1) + "-C" + (coord.col + 1) + ") and its surrounding boxes have been stamped to letter \"" + targetLetter + "\". The new state of the box grid:\n");
                        printGrid();
                        return;
                    }
                ...
                while (true) {
                    ...
                    if (coord != null && isValidCoordinate(coord.row, coord.col)) {
                        acquireAndUseTool(tool, coord.row, coord.col);
                        System.out.println("\nThe chosen box on location R" + (coord.row + 1) + "-C" + (coord.col + 1) + " has been flipped upside down. The new state of the box grid:\n");
                        printGrid();
                        return;
                    }
                    System.out.println("Invalid location. Please use R#-C# format.");
                ...
                while (true) {
                    ...
                    if (coord != null && isValidCoordinate(coord.row, coord.col)) {
                        acquireAndUseTool(tool, coord.row, coord.col);
                        System.out.println("\nThe chosen box on location R" + (coord.row + 1) + "-C" + (coord.col + 1) + " has been replaced with a FixedBox. The new state of the box grid:\n");
                        printGrid();
                        return;
                    }
                    System.out.println("Invalid location. Please use R#-C# format.");
        ...
        private void printGameEnd() {
            ...
            System.out.println("THE TOTAL NUMBER OF TARGET LETTER \"" + targetLetter + "\" IN THE BOX GRID --> " + score);
