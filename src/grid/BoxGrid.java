 ...
 * This class handles the core game physics including the "Domino Effect"
 * rolling mechanism,
...
public class BoxGrid {
     ...
     * Required for rule validation: "Player must select a box that was rolled in
     * the previous stage."
     */
    ...
    /**
     * Fills the grid with 64 randomly generated boxes according to probability
     * rules.
     */
     ...
     * @throws UnmovableFixedBoxException If the user tries to push a FixedBox
     *                                    directly.
     */
    public boolean rollBox(int row, int col, Direction dir) throws UnmovableFixedBoxException {
        ...
        if (!isValidMoveFromEdge(row, col, dir))
            return false;
        ...
        switch (dir) {
            ...
            case DOWN:
                return shiftColDown(col);
            case UP:
                return shiftColUp(col);
        }
        return false;
    }
     ...
     * Handles corners (which are on two edges) and regular edges.
     */
    private boolean isValidMoveFromEdge(int r, int c, Direction dir) {
        // Check corners first (corners are on two edges, so they have two valid
        // directions)
        if (r == 0 && c == 0) {
            // Top-left corner: can move DOWN or RIGHT
            return dir == Direction.DOWN || dir == Direction.RIGHT;
        }
        if (r == 0 && c == SIZE - 1) {
            // Top-right corner: can move DOWN or LEFT
            return dir == Direction.DOWN || dir == Direction.LEFT;
        }
        if (r == SIZE - 1 && c == 0) {
            // Bottom-left corner: can move UP or RIGHT
            return dir == Direction.UP || dir == Direction.RIGHT;
        }
        if (r == SIZE - 1 && c == SIZE - 1) {
            // Bottom-right corner: can move UP or LEFT
            return dir == Direction.UP || dir == Direction.LEFT;
        }

        // Check regular edges (not corners)
        if (r == 0)
            return dir == Direction.DOWN; // Top edge
        if (r == SIZE - 1)
            return dir == Direction.UP; // Bottom edge
        if (c == 0)
            return dir == Direction.RIGHT; // Left edge
        if (c == SIZE - 1)
            return dir == Direction.LEFT; // Right edge
     ...
     * 3. The box immediately to the left of the FixedBox is "crushed/eaten"
     * (overwritten).
    ...
    private boolean shiftRowRight(int row) throws UnmovableFixedBoxException {
        ...
        // 2 & 3. Shift Logic (Iterate backwards to prevent overwriting data we still
        // need)
    ...
    /**
     * Checks if a box at specific coordinates was moved during the last roll
     * operation.
    ...
    public void convertToFixedBox(int row, int col) throws BoxAlreadyFixedException {
        ...
        // Replace with a new FixedBox (effectively removing any tool inside the old
        // box)
    ...
    /**
     * Calculates the final score by counting how many boxes have the target letter
     * on top.
     */
